...
local function UpdateBlips()
    ...
    for k, v in pairs(players) do
        if (v.PlayerData.job.name == "police" or v.PlayerData.job.name == "firefighter") and v.PlayerData.job.onduty then
...
QBCore.Commands.Add("depot", "Send to depot (Police Only)", {{name = "price", help = "Leave this empty"}}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
RegisterNetEvent("police:server:syncspeedzones", function(x, y, z, zone, blip)
    local players = QBCore.Functions.GetPlayers()

    for k, v in pairs(players) do
        local src =  QBCore.Functions.GetPlayer(v)
        TriggerClientEvent("police:client:syncspeedzones", src, x, y, z, zone, blip)
    end
end)
